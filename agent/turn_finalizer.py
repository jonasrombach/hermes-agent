...
def _apply_output_hooks(
    ...
    for _hook_result in _invoke_hook_safely(
        ...
        if isinstance(_hook_result, str) and _hook_result:
            ...
            break
    # Detached/private work must not publish turns under the user's session ID.
    if not private_turn and not getattr(agent, "_persist_disabled", False):
        _invoke_hook_safely(
...
def finalize_turn(
    ...
    if getattr(agent, "_gateway_private_turn", False):
        start = next((i for i in range(len(messages) - 1, -1, -1) if messages[i].get("role") == "user"), len(messages))
        for message in messages[start:]:
            if isinstance(message, dict): message["hermes_private_turn"] = True
    else:
        _guarded_cleanup(
            "save_trajectory",
    ...
    def _persist_step():
        ...
        if not interrupted and not failed and not getattr(agent, "_gateway_private_turn", False):
    ...
    if not getattr(agent, "_gateway_private_turn", False):
        with suppress(Exception):
    ...
    try:
        if getattr(agent, "_gateway_private_turn", False):
            raise RuntimeError("private turn observer suppressed")
