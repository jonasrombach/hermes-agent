...
class GatewayInboundMixin:
    ...
    async def _hm_handle_running_session_message(
        ...
        if not getattr(event, "internal", False) and getattr(running_agent, "_gateway_private_turn", False) is True:
            self._queue_or_replace_pending_event(_quick_key, event)  # type: ignore[reportAttributeAccessIssue]  # mixed in by GatewayBusySessionMixin
            return None
        if effective_busy_input_mode == "queue":
            ...
            try:
                ...
                if plugin_handler:
                    result = invoke_plugin_command(
                        ...
                        PluginCommandContext(session_key=build_session_key(source)),
                    )
    ...
    async def _dispatch_plugin_message_injection(
        ...
        event = MessageEvent(text=content, message_type=MessageType.TEXT, source=source, internal=True,
        ...
        try:
            ...
            logger.warning(
                "Plugin message injection failed: plugin=%s session=%s", plugin_id, session_key,
                exc_info=True,
            )
            return False
        logger.info(
