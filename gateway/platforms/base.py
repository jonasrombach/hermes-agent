...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Private wakes remain isolated, but their head shares a monotonic
        # ingress order with public input at the adapter execution boundary.
        self._pending_private_messages: Dict[str, List[MessageEvent]] = {}
        self._pending_ingress_order = 0
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        for bucket in (self._background_tasks, self._expected_cancelled_tasks, self._session_tasks,
                       self._pending_messages, self._pending_private_messages, self._active_sessions, self._text_debounce_store()):
            bucket.clear()
    ...
    def _stamp_pending_order(self, event: MessageEvent) -> None:
        if not isinstance(getattr(event, "_hermes_pending_order", None), int):
            self._pending_ingress_order += 1
            event._hermes_pending_order = self._pending_ingress_order
