...
class GatewayBusySessionMixin:
    ...
    def _promote_queued_event(
        ...
        if adapter is not None and hasattr(adapter, "_pending_messages") and not bool(
            (getattr(pending_event, "metadata", None) or {}).get("hermes_private_turn")
        ):
    ...
    def _queue_or_replace_pending_event(self, session_key: str, event: MessageEvent) -> None:
        ...
        stamp = getattr(adapter, "_stamp_pending_order", None)
        if callable(stamp): stamp(event)
