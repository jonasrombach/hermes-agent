...
class TurnRunner:
    ...
    def run_sync(self):
        ...
        # The cache spans turns: refresh routing identity and clear/set private scope each time.
        agent._gateway_private_turn = bool(ctx.private_turn)
        agent._gateway_session_key = ctx.session_key
        for name in ("user_id", "user_id_alt", "user_name", "chat_id", "chat_name", "chat_type", "thread_id"):
            setattr(agent, "_" + name, getattr(ctx.source, name, None))
