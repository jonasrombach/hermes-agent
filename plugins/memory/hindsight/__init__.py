...
def _prepare_recall_query(query: str, max_chars: int) -> str:
    """Skip trivial acknowledgements and bound long queries head+tail."""
...
def _format_recall_result(result: Any) -> str:
    """Render a recalled memory with temporal provenance when available."""
    details = []
    for name, label in (("type", "type"), ("mentioned_at", "mentioned"), ("id", "fact"), ("document_id", "document")):
        if value := getattr(result, name, None):
            details.append(f"{label}={value}")
    start, end = getattr(result, "occurred_start", None), getattr(result, "occurred_end", None)
    if start:
        details.append(f"occurred={start}" + (f"..{end}" if end and end != start else ""))
    values = getattr(result, "source_fact_ids", None)
    if values:
        shown = [str(value) for value in values[:5]]
        if len(values) > len(shown):
            shown.append(f"+{len(values) - len(shown)} more")
        details.append("sources=" + ",".join(shown))
    return (getattr(result, "text", "") or "") + (f" [{'; '.join(details)}]" if details else "")
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._bank_id, self._budget, self._auto_recall_budget, self._bank_id_template = "hermes", "mid", "mid", ""
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "prefer_observations", "description": "Suppress raw facts already represented by returned observations", "default": False},
            {"key": "recall_min_scores", "description": "Optional Hindsight per-stage score floors, e.g. {'final': 0.3}", "default": None},
            {"key": "auto_recall_budget", "description": "Search-depth budget for automatic per-turn recall (low/mid/high)", "default": ""},
    ...
    def _apply_connection_settings(self, cfg: dict) -> None:
        ...
        auto_budget = cfg.get("auto_recall_budget") or self._budget
    ...
    def _apply_recall_settings(self, cfg: dict) -> None:
        ...
        min_scores = cfg.get("recall_min_scores")
        self._recall_min_scores = dict(min_scores) if isinstance(min_scores, dict) else None
    ...
    def _recall(self, query: str) -> list:
        kwargs: dict = {"bank_id": self._bank_id, "query": query, "budget": self._auto_recall_budget, "max_tokens": self._recall_max_tokens}
    ...
    def _reflect(self, query: str) -> str | None:
        resp = self._run_hindsight_operation(
            lambda client: client.areflect(bank_id=self._bank_id, query=query, budget=self._auto_recall_budget)
        )
        return resp.text
    ...
    def _do_recall(self, query: str) -> tuple[str, int]:
        ...
        try:
            ...
            return "\n".join(f"- {_format_recall_result(r)}" for r in results if getattr(r, "text", "")), len(results)
        except Exception as e:
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        if self._recall_sync or self._recall_disabled() or is_trivial_prompt(query):
            return
