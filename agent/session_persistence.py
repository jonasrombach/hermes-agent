...
class SessionPersistenceMixin:
    ...
    def _flush_messages_to_session_db_unlocked(
        ...
        if getattr(self, "_gateway_private_turn", False) or getattr(self, "_persist_disabled", False) or not self._session_db:
            return None
        batch_rows: List[Dict[str, Any]] = []
        try:
            ...
            keep = [(row, msg) for row, msg in zip(batch_rows, batch_msgs) if not msg.get("hermes_private_turn")]
            batch_rows, batch_msgs = map(list, zip(*keep)) if keep else ([], [])
